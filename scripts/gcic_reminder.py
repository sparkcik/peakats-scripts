...
"""
gcic_reminder.py
Automated 3-day reminder cadence for candidates who received GCIC outreach
but have not yet completed the GCIC form.

Day 1: Template 8 (gentle nudge)
Day 2: Template 9 (firmer follow-up)
Day 3: Template 10 (final reminder + escalation to PEAK Ops)

Runs on forge-local (Fly.io) via forge_runner.py scheduler.

Usage:
    python3 scripts/gcic_reminder.py            # process all eligible
    python3 scripts/gcic_reminder.py --dry-run   # preview without sending
"""
...
# -- Main ---------------------------------------------------------------------
def run_gcic_reminder():
    print(f'[GCIC Reminder] {"DRY RUN -- " if DRY_RUN else ""}Starting at {datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")}')

    # Fetch candidates with GCIC outreach sent but form not completed
    candidates = sb_get('candidates', {
        'select': 'id,first_name,last_name,client_id,phone,gcic_reminder_count,gcic_last_reminder_date,gcic_outreach_sent_at',
        'gcic_outreach_sent_at': 'not.is.null',
    ...
    print(f'[GCIC Reminder] {len(candidates)} candidate(s) in reminder pool.')

    if not candidates:
        print('[GCIC Reminder] Nothing to send. Exiting.')
        return

    # Pre-fetch templates
    templates = {}
    for tpl_id in (8, 9, 10):
        body = get_template_body(tpl_id)
        if body:
            templates[tpl_id] = body
        else:
            print(f'[GCIC Reminder] WARNING: Template {tpl_id} not found in message_templates.')
    ...
    for c in candidates:
        ...
        reminder_count = c.get('gcic_reminder_count') or 0
        last_reminder = c.get('gcic_last_reminder_date')
        outreach_sent = c.get('gcic_outreach_sent_at')
        ...
        # Update candidate
        update_fields = {
            'gcic_reminder_count': reminder_count + 1,
            'gcic_last_reminder_date': now_iso,
            'updated_at': now_iso
        }

        # Day 3 escalation
        if tpl_id == 10:
            update_fields['gcic_escalated_at'] = now_iso
            try:
                sb_insert('forge_memory', {
                    'category': 'ops_note',
                    'subject': f'GCIC Day 3 Escalation - {first} {last}',
                    'target_thread': 'PEAK Ops',
                    'content': f'Candidate {cid} ({first} {last}) client={client} phone={phone} -- 3 GCIC reminders sent with no form completion. Escalating.',
                    'created_at': now_iso
                })
            except Exception as e:
    ...
    summary = f'GCIC Reminder run at {now_str}: {count} sent, {skipped} skipped out of {len(candidates)} eligible.'
    print(f'\n[GCIC Reminder] Done. {count} queued, {skipped} skipped.')

    if not DRY_RUN and count > 0:
        try:
            sb_insert('forge_memory', {
                'category': 'ops_note',
                'subject': f'GCIC Reminder Run Summary - {now_str}',
                'target_thread': 'PEAK Ops',
                'content': summary,
                'created_at': now_iso
            })
        except Exception as e:
            print(f'[GCIC Reminder] FAILED to log summary to forge_memory: {e}')
